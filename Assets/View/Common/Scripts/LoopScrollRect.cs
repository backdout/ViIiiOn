...
/*
 * https://github.com/qiankanglai/LoopScrollRect 참조
 * 
 * 스크롤 뷰는 LoopVerticalScrollRect또는 LoopHorizontalScrollRect로 사용한다.
 * 
 * 스크롤 아이템은 public void SetData(object data) 함수를 만들고 
 * data를 필요 데이터 형식에 맞게 캐스팅 하여 사용한다
 * 
 * 스크롤 아이템에Layout Element 컴포넌트를 추가해서 PreferredWidth, PreferredHeight를 지정해준다 
 * 
 * Contetnt에는 ContentSizeFitter와 VerticalLayoutGroup또는 HorizontalLayoutGroup를 추가한다 
 * 
 * **/
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        public void UpdateCells()
        {
            ...
            {
                dataSource.UpdateData(content.GetChild(i));
            }
        }

        public void UpdateCells(object value)
        {
            for (int i = 0; i < content.childCount; i++)
            {
                dataSource.UpdateData(content.GetChild(i), value);
            }
        }


        public void AddDatas(object obj, int count)
        {
            totalCount += count;
            dataSource.AddData(obj);
        }

        public void init(object[] list)
        {
            if (!prefabSource.inited)
                prefabSource.setPool(this.transform);

            totalCount = list.Length;
            objectsToFill = new List<object>(list);
            RefillCells();
        ...
        // 스크롤 이동없이 초기화
        public void Reinit(object[] list)
        {
            if (totalCount == 0)
            {
                init(list);
        ...
        public List<object> objectsToFill
        {
        ...
        public void ReRefillCells()
        {
            ...
            //Debug.Log("itemTypeEnd:" + itemTypeEnd);
            itemTypeEnd = itemTypeStart;
            ...
            float size = 0.0f;
            int Count = 0;
            while (sizeToFill > sizeFilled || ChildCount >= Count)
            {
                size = reverseDirection ? NewItemAtStart() : NewItemAtEnd();
                Count++;
                if (size <= 0) break;
                sizeFilled += size;
            }
        }
        public void RewardRefillCell()
        {
            if (!Application.isPlaying || prefabSource == null)
                return;
            StopMovement();
        ...
        protected float DeleteItemAtStart()
        {
            ...
            if (((isDragging || m_Velocity != Vector2.zero) && totalCount >= 0 && itemTypeEnd >= totalCount - 1)
        ...
        {
            if (((isDragging || m_Velocity != Vector2.zero) && totalCount >= 0 && itemTypeStart < contentConstraintCount)
                || content.childCount == 0)
            {
                return 0;
            }
        ...
        private RectTransform InstantiateNextItem(int itemIdx)
        {
            ...
            nextItem.gameObject.ResetTransform();
        ...
        public virtual void StopMovementImmediately()
        {
            ...
            //m_ContentBounds.min = m_ViewBounds.min;
            //m_ContentBounds.max = m_ViewBounds.max;

            isStopMovement = true;
            StopAllCoroutines();
            //inertia = false;
            //Invoke("onMovement", 1f);
        }
        ...
        protected virtual void LateUpdate()
        {
            ...
            if (!isDragging && (offset != Vector2.zero || m_Velocity != Vector2.zero) && isStopMovement == false)
            {
                Vector2 position = m_Content.anchoredPosition;
        ...
        private Vector2 CalculateOffset(Vector2 delta)
        {
            ...
            if (m_Vertical)
            {
                ...
                //Debug.Log("m_ContentBounds min:" + m_ContentBounds.min.y);
                //Debug.Log("m_ContentBounds max:" + m_ContentBounds.max.y);
                //Debug.Log("m_ViewBounds min:" + m_ViewBounds.min.y);
                //Debug.Log("m_ViewBounds max:" + m_ViewBounds.max.y);
            }
