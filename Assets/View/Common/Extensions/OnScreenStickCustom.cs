...
public class OnScreenStickCustom : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    public UnityEvent onJoystickRelease = new UnityEvent();
    ...
    private void BeginInteraction(Vector2 pointerPosition, Camera uiCamera)
    {
        ...
        if (canvasRect == null)
        {
            Debug.LogError("OnScreenStickCustom needs to be attached as a child to a UI Canvas to function properly.");
            return;
        }
...
#if UNITY_EDITOR
    [CustomEditor(typeof(OnScreenStickCustom))]
    internal class OnScreenStickCustomEditor : UnityEditor.Editor
    {
        ...
        public void OnEnable()
        {
            ...
            m_UseIsolatedInputActions = serializedObject.FindProperty(nameof(OnScreenStickCustom.m_UseIsolatedInputActions));

            m_Behaviour = serializedObject.FindProperty(nameof(OnScreenStickCustom.m_Behaviour));
            m_MovementRange = serializedObject.FindProperty(nameof(OnScreenStickCustom.m_MovementRange));
            m_DynamicOriginRange = serializedObject.FindProperty(nameof(OnScreenStickCustom.m_DynamicOriginRange));
            m_PointerDownAction = serializedObject.FindProperty(nameof(OnScreenStickCustom.m_PointerDownAction));
            m_PointerMoveAction = serializedObject.FindProperty(nameof(OnScreenStickCustom.m_PointerMoveAction));
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            m_ShowDynamicOriginOptions.target = ((OnScreenStickCustom)target).behaviour ==
            ...
            if (EditorGUILayout.BeginFadeGroup(m_ShowDynamicOriginOptions.faded))
            {
                ...
                {
                    ((OnScreenStickCustom)target).UpdateDynamicOriginClickableArea();
                }
                EditorGUI.indentLevel--;
            }
